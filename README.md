...
这是一个基于 PyTorch 的小型强化学习项目：智能体在 GridWorld 网格地图中学习从起点移动到目标点，同时避开障碍物。项目使用 DQN（Deep Q-Network）算法，适合作为 GitHub 展示、简历项目和强化学习入门练习。
...
- 设计并实现 GridWorld 强化学习环境，包括智能体位置、目标点、障碍物、奖励函数、状态表示、动作空间和回合终止条件。
- 基于 PyTorch 实现 DQN 智能体，包含经验回放、epsilon-greedy 探索策略、目标网络更新和模型保存/加载机制。
- 记录训练奖励、成功率、步数和 loss 等指标，输出训练曲线与测试轨迹，用于分析奖励设计和探索率衰减对智能体表现的影响。
